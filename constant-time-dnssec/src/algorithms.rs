...
/// Verify an Ed25519 signature in constant time
///
...
pub fn verify_ed25519(sig: &DnssecSignature, data: &SignedData) -> CtVerificationResult {
    ...
    let public_key = match VerifyingKey::from_bytes(sig.public_key.as_ref().try_into().unwrap_or(&[0u8; 32])) {
        Ok(key) => key,
    ...
    // Verify - ed25519-dalek's verify is designed to be constant-time
    // It handles SHA-512 internally
    match public_key.verify(&signed_data, &signature) {
        Ok(_) => CtVerificationResult::success(),
...
pub fn verify_ecdsa_p256(sig: &DnssecSignature, data: &SignedData) -> CtVerificationResult {
    ...
    // Verify - p256's verify is designed to be constant-time
...
pub fn verify_rsa_sha256(sig: &DnssecSignature, data: &SignedData) -> CtVerificationResult {
    ...
    let public_key = match RsaPublicKey::from_pkcs1_der(sig.public_key.as_ref()) {
        Ok(key) => key,
        Err(_) => return CtVerificationResult::failure(),
    };
    ...
    // Parse signature
    let signature = match rsa::pkcs1v15::Signature::try_from(sig.signature.as_ref()) {
        Ok(sig) => sig,
        Err(_) => return CtVerificationResult::failure(),
    };
...
mod tests {
    ...
    // Note: These tests would require generating actual test keys
    // For now, we test the error handling paths

    #[test]
    fn test_ed25519_invalid_signature() {
        // Use a valid-looking key but invalid signature
        let sig = DnssecSignature {
            algorithm: DnssecAlgorithm::Ed25519,
            signature: bytes::Bytes::from(vec![0xffu8; 64]), // Invalid signature
            public_key: bytes::Bytes::from(vec![0x01u8; 32]), // Non-zero key
        };
        let data = SignedData {
            rrset_data: bytes::Bytes::from("test message for verification"),
            rrsig_header: bytes::Bytes::from("header"),
        };
        let result = verify_ed25519(&sig, &data);
        assert!(!result.is_valid());
    }

    #[test]
    fn test_ecdsa_p256_invalid_key() {
        let sig = DnssecSignature {
            algorithm: DnssecAlgorithm::EcdsaP256Sha256,
            signature: bytes::Bytes::from(vec![0u8; 64]),
            public_key: bytes::Bytes::from(vec![0u8; 65]),
        };
        let data = SignedData {
            rrset_data: bytes::Bytes::from("test"),
            rrsig_header: bytes::Bytes::from("header"),
        };
        let result = verify_ecdsa_p256(&sig, &data);
        assert!(!result.is_valid());
    }

    #[test]
    fn test_rsa_invalid_key() {
        let sig = DnssecSignature {
            algorithm: DnssecAlgorithm::Rsasha256,
            signature: bytes::Bytes::from(vec![0u8; 256]),
            public_key: bytes::Bytes::from(vec![0u8; 256]),
        };
        let data = SignedData {
            rrset_data: bytes::Bytes::from("test"),
            rrsig_header: bytes::Bytes::from("header"),
        };
        let result = verify_rsa_sha256(&sig, &data);
        assert!(!result.is_valid());
    }
}
